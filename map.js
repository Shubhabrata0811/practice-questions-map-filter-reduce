...
const firstCharactersOf = function (strings) {
  return strings.map((str) => (str.length === 0 ? "" : str[0]));
};
...
const getDoubleLetteredStr = (str) =>
  Array.from(str).reduce((updatedStr, char) => updatedStr + char.repeat(2), "");
...
const charCodesOf = function (strings) {
  return strings.map((str) => str.charCodeAt(0));
};
...
const joinedArraysOf = function (arrayOfArrays) {
  return arrayOfArrays.map((arr) => arr.join(""));
};

// repeat strings in ["hi", "bye"] => ["hihi", "byebye"]
const repeatedStringsOf = function (strings) {
  return strings.map((str) => str.repeat(2));
};
...
const reversedArraysOf = function (arrays) {
  return arrays.map((arr) => arr.reverse((a, b) => b - a));
};
...
const uniqueCharactersOf = function (strings) {
  return strings.map((str) =>
    [...str].reduce(
      (uniqueStr, char) => uniqueStr + (uniqueStr.includes(char) ? "" : char),
      ""
    )
  );
};
...
const wordLengthsOf = function (strings) {
  return strings.map((str) => lengthsOf(str.split(" ")));
};

// flatten nested arrays of [[1, [2, 3]], [4, [5, 6]]] => [[1, 2, 3], [4, 5, 6]]
const flattenedArraysOf = function (arrays) {
  return arrays.map((arr) => arr.flat());
};
...
const firstLettersOfNames = function (objects) {
  return objects.map((obj) => (obj.name.length === 0 ? "" : obj.name[0]));
};
...
const extractFlags = function (objects) {
  return objects.map((obj) => obj.active);
};
...
const normalizeScores = (objects) =>
  objects.map(
    ({ score }) => score / Math.max(...objects.map(({ score }) => score))
  );
...
const percentageContributions = (numbers) =>
  numbers.map(
    (num, idx, arr) =>
      +((num * 100) / arr.reduce((sum, num) => sum + num, 0)).toFixed(2)
  );
